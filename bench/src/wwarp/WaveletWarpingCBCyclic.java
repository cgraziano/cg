...
public class WaveletWarpingCBCyclic {
    ...
    float[] u, float[] f, float[] g, int niter)
  {
    ...
    _arraySizeNiter = niter;
    _allResRmsAllS = new float[_arraySizeNiter];
   ...
   */
  public float[] getShapingFilterH(
    ...
    float[] u, float[] f, float[] g)
  {
    ...
    //SymmetricToeplitzDMatrix stm = new SymmetricToeplitzDMatrix(qq);
    //return convertDToF(stm.solve(qf));
  }
  /**
   * Estimates that shaping filter that will shape SBg to f.
   * @param nc number of samples in wavelet c.
    ...
    float[] u, float[] f, float[] g)
  {
    ...
    */
    ...
    //Note that this is the code to quickly construct a Toeplitz matrix quickly and
    //solve this Toeplitz system with a fast Toeplitz solver.
    //Note that although this method is faster, it is less stable than solving this 
    //system with Cholesky decomposition.
    //In my synthetics, I found that a more accurate solution is reached with Cholesky decomposition 
    //rather than the fast Toeplitz solver (Levinson Recursion)
    ...
    f = copy(mt,_itmin,f);
    q0 = copy(mt,_itmin,q0);
    xcor(mt,0,q0,mt,0,f,nc,kc,cqf);
    xcor(mt,0,q0,mt,0,q0,nc, 0,cqq);

    cqq[0] *= (1.0+_sfac);

    //Solve for wavelet h.
    SymmetricToeplitzFMatrix stm = new SymmetricToeplitzFMatrix(cqq);
    float[] vv = stm.solve(cqf);
    return vv;
  }
    ...
    float[][] u, float[][] f, float[][] g)
  {
    ...
    */
    ...
    float[] cqf = new float[nc];
    float[] cqq = new float[nc];
    float[] tqf = new float[nc];
    float[] tqq = new float[nc];
    int nx = f.length;
    int mt = (0<_itmin && _itmin<_itmax && _itmax<nt)?1+_itmax-_itmin:nt;
    f = copy(mt,nx,_itmin,0,f);
    q0 = copy(mt,nx,_itmin,0,q0);
    for (int ix=0; ix<nx; ++ix) {
      xcor(mt,0,q0[ix],mt,0,f[ix],nc,kc,tqf);
      xcor(mt,0,q0[ix],mt,0,q0[ix],nc, 0,tqq);
