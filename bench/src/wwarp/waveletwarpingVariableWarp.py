...
def goSimpleTest():
  nt = 481 # number of time samples; number of impulses
  ...
  for mp in [False]: # True, for minimum-phase; False for other
    ...
    it1,it2 = 50,200
    r = [2.0,1.3]
    ird = 100 #index that divides the two zones of r, at ird r is 2nd r
    rmax = 1.9 
    nr = len(r)
    p,q = makeImpulsesVariable(it1,it2,ird,r,nt)
    ...
    rzone = (nt+nr)/nr
    u = zerofloat(nt)
    for i in range(0,ird):
      u[i] = i*r[0]
    for i in range(ird,nt):
      u[i] = i*r[1]
    #u[ird-5] = u[ird-6]+1.9
    #u[ird-4] = u[ird-5]+1.8
    #u[ird-3] = u[ird-4]+1.7
    ...
    #u[ird] = u[i]+r[1]
    #uird = u[ird]
    #for i in range(ird+1,nt):
      #u[i] = u[i-1]+r[1] 

    for i in range(1,nt):
      print str(u[i]-u[i-1])

    fmin,fmax = 0.0,min(0.5,0.5*r[nr-1]) # bandpass (lowpass), if stretching
    ww = WaveletWarping()
    ...
    q = ww.applyB(q)
    p = ww.applyB(p)
    lg = ww.applyLVariableS(r[0],u,g)
    slg = ww.applyS(u,g)
    #slg[100]=0
    ak = ww.getWaveletH(nh,kh,hk,na,ka) # known inverse wavelet
    aw = ww.getInverseAVariable(na,ka,u,rmax,f,g) # estimated inverse wavelet
    SimplePlot.asPoints(aw)
    hw = ww.getWaveletH(na,ka,aw,nh,kh) # estimated wavelet
    dump(aw)
    dump(hw)
    sg = ww.applyS(u,g)
    af = ww.applyA(na,ka,ak,f)
    ag = ww.applyA(na,ka,ak,g)
    lag = ww.applyLVariableS(r[0],u,ag) # lowpass, if squeezing
    slag = ww.applyS(u,lag)
    #slag[100]=0
    baf = ww.applyB(af)
    bslag = ww.applyB(slag)
    hslag = ww.applyH(nh,kh,hk,slag)
    nhw = normalize(hw)
    nhk = normalize(hk)
    title = "r = "+str(r)
  ...
  for mp in [False]: # True, for minimum-phase; False for other
    ...
    for r in [.5]: # for stretch and squeeze, ...
      for sfac in [1.00,1.00001,1.0001,1.001,1.01,1.1,1.5]:
        ...
        lag = ww.applyLVariableS(u,ag) # lowpass, if squeezing
        slag = ww.applyS(u,lag)
        baf = ww.applyB(af)
        bslag = ww.applyB(slag)
...
def makeImpulsesVariable(it1,it2,ird,r,nt):
  p = zerofloat(nt)
  q = zerofloat(nt)
  itmax = nt-1
  ts = [it1,it2]
  ...
  for ji in range(ni):
    ...
    si.accumulate(tq,rj,nt,1.0,0.0,q)
  return p,q
#def makeImpulsesVariable(it1,it2,it3,r,exp,nt):
#  p = zerofloat(nt)
#  q = zerofloat(nt)
#  itmax = nt-1
#  ts = [it1,it2,it3]
