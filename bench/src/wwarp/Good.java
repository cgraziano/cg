...
public class Good {
    ...
    float[] u, float[] f, float[] g, int niter)
  {
    ...
    _rmsritotal = new float[niter];//db
    _rmsridata = new float[niter];//db
    _rmsripc = new float[niter];//db
    _rmsripb = new float[niter];//db
    _rmsripdeltac = new float[niter];//db
    _rmsripdeltab = new float[niter];//db
    _rmsrftotal = new float[niter];//db
    _rmsrfdata = new float[niter];//db
    _rmsrfpc = new float[niter];//db
    _rmsrfpb = new float[niter];//db
    _rmsrfpdeltac = new float[niter];//db
    _rmsrfpdeltab = new float[niter];//db
    ...
    float[] ridata = new float[f.length];
    float[] ripenaltyb = new float[nb];
    float[] ripenaltyc = new float[nc];
    float[] ripenaltydeltab = new float[nb];
    float[] ripenaltydeltac = new float[nc];
    float[] rfdata = new float[f.length];
    float[] rfpenaltyb = new float[nb];
    float[] rfpenaltyc = new float[nc];
    float[] rfpenaltydeltab = new float[nb];
    float[] rfpenaltydeltac = new float[nc];
    _zc = new float[nc];
    _wb = new float[nb];
    _zdeltac = new float[nc];
    _wdeltab = new float[nb];
    float rmsritotal = 0.0f;
    float rmsrid = 0.0f;
    float rmsripc = 0.0f;
    float rmsripb = 0.0f;
    float rmsripdc = 0.0f;
    ...
    for (int iter=0; iter<niter; ++iter) {
      ...
      ripenaltyc = computePenaltyCResidual(c);
      ripenaltyb = computePenaltyBResidual(b);
      ripenaltydeltab = computePenaltyDeltaBResidual(deltab);
      ripenaltydeltac = computePenaltyDeltaCResidual(deltac);
      rmsrid = rms(ridata);
      rmsripc = rmsAll(ripenaltyc);
      rmsripb = rmsAll(ripenaltyb);
      rmsripdc = rmsAll(ripenaltydeltac);
      rmsripdb = rmsAll(ripenaltydeltab);
      rmsritotal = rmsrid+rmsripc+rmsripb+rmsripdc+rmsripdb;
      ...
      */
      ...
      rfdata = computeDataResidual(nc,kc,cnew,nb,kb,bnew,u,f,g);
      rfpenaltyc = computePenaltyCResidual(cnew);
      rfpenaltyb = computePenaltyBResidual(bnew);
      rfpenaltydeltab = computePenaltyDeltaBResidual(deltab);
      rfpenaltydeltac = computePenaltyDeltaCResidual(deltac);
      rmsrfd = rms(rfdata);
      rmsrfpc = rmsAll(rfpenaltyc);
      rmsrfpb = rmsAll(rfpenaltyb);
      rmsrfpdc = rmsAll(rfpenaltydeltac);
      rmsrfpdb = rmsAll(rfpenaltydeltab);
      rmsrftotal = rmsrfd+rmsrfpc+rmsrfpb+rmsrfpdc+rmsrfpdb;
      rmspercentchange = percentChange(rmsrftotal,rmsritotal);

      //Debugging Information
      _twonormb[iter] = twoNorm(b);
      _twonormc[iter] = twoNorm(c);
      _lastiter = iter;
      _rmsritotal[iter] = rmsritotal;
      _rmsridata[iter] = rmsrid; 
      _rmsripc[iter] = rmsripc;
      _rmsripb[iter] = rmsripb; 
      _rmsripdeltac[iter] = rmsripdc;
      _rmsripdeltab[iter] = rmsripdb;
      _rmsrftotal[iter] = rmsrftotal;
      _rmsrfdata[iter] = rmsrfd; 
      _rmsrfpc[iter] = rmsrfpc;
      _rmsrfpb[iter] = rmsrfpb; 
      _rmsrfpdeltac[iter] = rmsrfpdc;
      _rmsrfpdeltab[iter] = rmsrfpdb;
      ...
      trace("initial rmsrtotal: "+rmsritotal);
      trace("final rmsrtotal: "+rmsrftotal);
      trace("initial rmsrdata: "+rmsrid);
      trace("final rmsrdata: "+rmsrfd);
      trace("initial rmsrpenaltyc: "+rmsripc);
      trace("final rmsrpenaltyc: "+rmsrfpc);
      trace("initial rmsrpenaltyb: "+rmsripb);
      trace("final rmsrpenaltyb: "+rmsrfpb);
      trace("initial rmsrpenaltydeltab: "+rmsripdb);
      trace("final rmsrpenaltydeltab: "+rmsrfpdb);
      trace("initial rmsrpenaltydeltac: "+rmsripdc);
      trace("final rmsrpenaltydeltac: "+rmsrfpdc);
      trace("rmsrtotal change: "+(rmsrftotal-rmsritotal));
  ...
  */
  ...
  public float[] getRMSRiTotal() {
    return _rmsritotal;
  }
  public float[] getRMSRiData() {
    return _rmsridata;
  }
  public float[] getRMSRiPenaltyC() {
    return _rmsripc;
  }
  public float[] getRMSRiPenaltyB() {
    return _rmsripb;
  }
  public float[] getRMSRiPenaltyDeltaC() {
    return _rmsripdeltac;
  }
  public float[] getRMSRiPenaltyDeltaB() {
    return _rmsripdeltab;
  }

  public float[] getRMSRfTotal() {
    return _rmsrftotal;
  }
  public float[] getRMSRfData() {
    return _rmsrfdata;
  }
  public float[] getRMSRfPenaltyC() {
    return _rmsrfpc;
  }
  public float[] getRMSRfPenaltyB() {
    return _rmsrfpb;
  }
  public float[] getRMSRfPenaltyDeltaC() {
    return _rmsrfpdeltac;
  }
  public float[] getRMSRfPenaltyDeltaB() {
    return _rmsrfpdeltab;
  }
...
///////////////////////////////////////////////////////////////////////////
  ...
  private float[] _rmsritotal;
  private float[] _rmsridata;
  private float[] _rmsripc;
  private float[] _rmsripb;
  private float[] _rmsripdeltac;
  private float[] _rmsripdeltab;
  private float[] _rmsrftotal;
  private float[] _rmsrfdata;
  private float[] _rmsrfpc;
  private float[] _rmsrfpb;
  private float[] _rmsrfpdeltac;
  private float[] _rmsrfpdeltab;
  ...
  private float[] _zc;
  private float[] _wb;
  private float[] _zdeltac;
  private float[] _wdeltab;
  ...
  {
    if (simpleStab == true) {
      x = penalizeDeltaB(nc,nb,kb,x,sfac);
      x = penalizeDeltaC(nb,nc,kc,x,sfac);
    }
    if (penb == true) {
  ...
  private DMatrix penalizeDeltaB(int nc, int nb, int kb, DMatrix x, double sfac) {
    ...
    for (int ib=0; ib<nb; ++ib) {
      ...
      ww[ib] = stabfactor;
      _wdeltab[ib] = (float) sqrt(stabfactor);
      trace("x = "+x.get(ib,ib));
      trace("sfacp1 = "+(1.0+_sfac));
      trace("sfacx = "+x.plus(DMatrix.diagonal(ww)).get(ib,ib));
  ...
  private DMatrix penalizeDeltaC(int nb, int nc, int kc, DMatrix x, double sfac) {
    ...
    for (int ic=0; ic<nc; ++ic) {
      ...
      zz[nb+ic] = stabfactor;
      _zdeltac[ic] = (float) sqrt(stabfactor);
    }
    ...
    DMatrix x, DMatrix v, double alpha) 
  {
    ...
    for (int ib=0; ib<nb; ++ib) {
      ...
      _wb[ib] = (float) (sqrt(normalize)*lag);
    }
    for (int ib=0; ib<nb; ++ib) {
    ...
    DMatrix x, DMatrix v, double alpha) 
  {
    ...
    for (int ic=0; ic<nc; ++ic) {
      ...
      _zc[ic] = (float) (sqrt(normalize)*lag);
    }
    for (int ic=0; ic<nc; ++ic) {
  ...
  private float[] computePenaltyBResidual(float[] b) {
    int nb = b.length;
  ...
  private float[] computePenaltyCResidual(float[] c) {
    int nc = c.length;
  ...
  private float[] computePenaltyDeltaBResidual(float[] deltab) {
    int nb = deltab.length;
    float[] r = new float[nb];
    for (int i=0; i<nb; ++i) {
      r[i] = deltab[i]*_wdeltab[i];
    }
    return r;
  }
  private float[] computePenaltyDeltaCResidual(float[] deltac) {
    int nc = deltac.length;
    float[] r = new float[nc];
    for (int i=0; i<nc; ++i) {
      r[i] = deltac[i]*_zdeltac[i];
    }
    return r;
  }
  ...
  private class ErrorFunction implements BrentMinFinder.Function {
        ...
        public double evaluate(double steplength) {
          ...
          float[] ridata = computeDataResidual(_nc,_kc,c,_nb,_kb,b,_u,_f,_g);
          float[] ripenaltyc = computePenaltyCResidual(c);
          float[] ripenaltyb = computePenaltyBResidual(b);
          float[] ripenaltydeltab = computePenaltyDeltaBResidual(deltab);
          float[] ripenaltydeltac = computePenaltyDeltaCResidual(deltac);
          double rid = dot(ridata,ridata);
          double ripc = dotAll(ripenaltyc,ripenaltyc);
          double ripb = dotAll(ripenaltyb,ripenaltydeltab);
          double ripdc = dotAll(ripenaltydeltac,ripenaltydeltac);
          double ripdb = dotAll(ripenaltydeltab,ripenaltydeltab);

          //trace("steplength = "+steplength);
          //trace("dotrr = "+dot(r,r));
          return rid+ripc+ripb+ripdc+ripdb;
        }
