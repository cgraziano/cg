...
from wwarp import WaveletWarping, WaveletWarpingAEig, Warper
...
def goEstimateWaveletTest():
  ...
  moreps = False 
  na,ka = 5,-2 #sampling for1inverse wavelet B #Note ka<=kc (B is in g)
    ...
    for ir1 in range(0,nr1):
      for iv in range(0,nv):
        for inoise in range(0,nnoise):
          ...
          #p,q,f,g,noisef,noiseg,u,tmin,tmax = createSyntheticLn1DTest(freqc,decayc,mpc,\
          ...
          cw = ww.getWaveletH(na,ka,bw,nc,kc)
          bcw = ww.applyA(na,ka,bw,cw)
          SimplePlot.asPoints(bcw)
          ...
          bk = ww.getWaveletH(nc,kc,ck,na,ka)
          ...
          sg = warp.applyS(u,g)
          sq = warp.applyS(u,q)
          sgnonoise = sub(warp.applyS(u,g),warp.applyS(u,noiseg))

          #deconvolve f
          bf = ww.applyA(na,ka,bw,f)
          
          #Create warped g
          bg = ww.applyA(na,ka,bw,g)
          sbg = warp.applyS(u,bg)
          hsbg = ww.applyH(nc,kc,cw,sbg)
          ...
          plotAmplitudeSpectrumT(Sampling(nt,1.0,0.0), f, 0, nt, "amp noisey f", amax=None)
          plotAmplitudeSpectrumT(Sampling(nt,1.0,0.0), sub(f,noisef), 0, nt, "amp f", amax=None)
          plotAmplitudeSpectrumT(Sampling(nt,1.0,0.0), g, 0, nt, "amp noisey g", amax=None)
          plotAmplitudeSpectrumT(Sampling(nt,1.0,0.0), sub(g,noiseg), 0, nt, "amp g", amax=None)
          plotAmplitudeSpectrumT(Sampling(nt,1.0,0.0), sub(f,sg), 0, nt, "amp noisey f-Sg", amax=None)
          plotAmplitudeSpectrumT(Sampling(nt,1.0,0.0), sub(sub(f,noisef),sgnonoise), 0, nt, "amp f-Sg", amax=None)
          #plotAmplitudeSpectrumT(Sampling(nt,1.0,0.0), noisef, 0, nt, "amp noisef", amax=None)
          plotAmplitudeSpectrumCkCwBkBw(1.0,0.0,[ck,cw,bk,bw],["amp ck","amp cw","amp bk","amp bw"], amax=None)
          if mpc:
            b0 = nbw[0]
            b1 = bw[1]
            b2 = bw[2]
            ...
            print "b2 = "+str(b2)
            real = -b1/(2*b0)
            imag = sqrt(abs(b1*b1-4.0*b0*b2))/(2*b0)
            rk = exp(-decayc)
            rw = sqrt(real*real+imag*imag)
            omegak = 2*PI*freqc
            omegaw = asin(imag/rw)
            print "rk = "+str(rk)
            print "rw = "+str(rw)
            print "omegak = "+str(omegak)
            print "omegaw = "+str(omegaw)
          ...
          plotWavelets(Sampling(na,dt,ka*dt),[nbw],title=title3,pngDir=pngDir,onecol=True)
          ...
          plotSequences(st,[p,q,f,g,hsbg,du],amax=amax,tmark=tmark,\
          labels=["p","q","f","g","HSBg","du"],pngDir=pngDir,\
          title=title)
...
def goEstimateWaveletTest2D():
  #Synthetic parameters
  nt,nx,ni,randomi = 481,1,55,True# number of time samples in p and q; number of random impulses in p and q.
  moreps = True 
  na,ka = 3,0#sampling for inverse wavelet A #Note ka<=kc 
  nc,kc = 181,-90 # sampling for wavelet H 
  v = [0.0]#The amount of shift between p and q.
  ...
  for ir0 in range(0,nr0):
    for ir1 in range(0,nr1):
      for iv in range(0,nv):
        ...
        title=title)
        """
def goEstimateWaveletNoiseTest():
  #Synthetic parameters
  nt,ni,randomi = 481,1,True# number of time samples in p and q; number of random impulses in p and q.
  moreps = False 
  na,ka = 3,0 #sampling for1inverse wavelet B #Note ka<=kc (B is in g)
  nc,kc = 181,0 # sampling for wavelet H 
  v = [0.0]#The amount of shift between p and q.
  r0,r1 = [2.0],[1.0]#constant u'(t) $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$2.0
  freqc,decayc = 0.08,0.05
  mpc = True#is wavelet in f mininmum phase?
  ...
  for ir0 in range(0,nr0):
    for ir1 in range(0,nr1):
      for iv in range(0,nv):
        for inoise in range(0,nnoise):
          #Create synthetic f and g.
          p,q,f,g,noisef,noiseg,u,tmin,tmax = synthetic.createSyntheticLn1DFilteredNoise(\
          knoiseupper,width,freqc,decayc,mpc,\
          freqd,decayd,mpd,r0[ir0],r1[ir1],v[iv],nrmsf[inoise],nrmsg[inoise],\
          nt,ni,randomi,moreps)
          #p,q,f,g,noisef,noiseg,u,tmin,tmax = synthetic.createSyntheticLn1D(freqc,decayc,mpc,\
          #freqd,decayd,mpd,r0[ir0],r1[ir1],v[iv],nrmsf[inoise],nrmsg[inoise],\
          ...
          hsbg = ww.applyH(nc,kc,cw,sbg)
          #getWarpingPartsSpectrums(r0[ir0],tmin,tmax,f,g,u)
          plotAmplitudeSpectrumT(Sampling(nt,1.0,0.0), f, 0, nt, "amp noisey f"+str(nrmsf[inoise]), amax=None)
          #plotAmplitudeSpectrumT(Sampling(nt,1.0,0.0), sub(f,noisef), 0, nt, "amp f", amax=None)
          plotAmplitudeSpectrumT(Sampling(nt,1.0,0.0), sg, 0, nt, "amp noisey Sg"+str(nrmsf[inoise]), amax=None)
          #plotAmplitudeSpectrumT(Sampling(nt,1.0,0.0), sub(g,noiseg), 0, nt, "amp g", amax=None)
          #plotAmplitudeSpectrumT(Sampling(nt,1.0,0.0), noisef, 0, nt, "amp noisef", amax=None)
          plotAmplitudeSpectrumCkCwBkBw(1.0,0.0,[ck,cw,bk,bw],["amp ck"+str(nrmsf[inoise]),"amp cw"+str(nrmsf[inoise]),"amp bk"+str(nrmsf[inoise]),"amp bw"+str(nrmsf[inoise])], amax=None)
          plotAmplitudeSpectrumT(Sampling(nt,1.0,0.0), sub(f,sg), 0, nt, "amp noisey f-Sg", amax=None)
          plotAmplitudeSpectrumT(Sampling(nt,1.0,0.0), sub(sub(f,noisef),sgnonoise), 0, nt, "amp f-Sg", amax=None)
          if mpc:
            bw = normalize1(bw)
            ...
            omegak = 2*PI*freqc
            omegaw = asin(imag/rw)
            print "nrmsf = "+str(nrmsf[inoise])
            print "rk = "+str(rk)
            print "rw = "+str(rw)
            print "omegak = "+str(omegak/(2.0*PI))
            print "omegaw = "+str(omegaw/(2.0*PI))
            bw1e,bw2e = -2.0*rw*cos(omegaw),rw*rw
            print "bw =",[b0,b1,b2]
            print "bwe =",[1,bw1e,bw2e]
...
def goSinopec1():
  ...
  print "tmin = "+str(tmin)
  print "tmax = "+str(tmax)
  ca = ww.applyH(nc,kc,cw,aw)
  SimplePlot.asPoints(ca)
  ...
  plotAmplitudeSpectrumT(Sampling(na,1.0,ka*1.0), naw, 0, na, "amp a", amax=None)
  ...
  amax = [maxf,max(sg),max(hsag),max(du)]
  tmark = [maxfd2,max(sg)/2.0,max(hsag)/2.0,0.1]
  plotSequences(st,[f,sg,hsag,du],amax=amax,tmark=tmark,\
  labels=["f","Sg","HSAg","du"])
...
def plotAmplitudeSpectrumT(st, p, itmin, itmax, title, amax=None):
  ...
  #Frequency sampling
  nfft = FftReal.nfftSmall(nt)#more time sample, the finer freq. samples
  nf = nfft/2+1
  df = 1.0/(nfft*dt)
  ff = 0.0
...
#Plot amplitude spectrum for four different wavelets
#Also returns the amplitude spectrum of bw
def plotAmplitudeSpectrumCkCwBkBw(dt, ft, ckcwbkbw, title, amax=None):
  #What is the largest length of the arrays cw, bw, ck, or bk?
  ntmax = 0
  for i in range(4):
    nt = len(ckcwbkbw[i])
    if (nt>ntmax):
  ...
  #Concatenate zeros onto arrays that don't have a length equal to ntmax.
  ckcwbkbwc = zerofloat(ntmax,4)
  for i in range(4):
    nt = len(ckcwbkbw[i])
    for j in range(nt):
      ckcwbkbwc[i][j] = ckcwbkbw[i][j]
  ...
  fs = Sampling(nf,df,ff)
  for i in range(4):
    amp = computeAmplitudeSpectrum(st,fs,nfft,ckcwbkbwc[i])
    #amp = div(amp,max(amp))
    plotSpectrum(fs,amp,title[i],amax=amax)
  print "nfft = "+str(nfft)
  print "df = "+str(df)
...
def getWarpingPartsSpectrums(r,itmin,itmax,f,g,u):
  ...
  plotAmplitudeSpectrumT(stf, f, 0, ng, "amp f", amax=None)
