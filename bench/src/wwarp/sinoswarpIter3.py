...
def enhancedWarping():
  ...
  r = 1.7
  #estimated inverse and wavelet parameters
  niter = 10
  eps = 5.0
  h = zerofloat(nh)
  h[-kh] = 1.00
...
##############StartTests######################
  ...
  #Find estimated inverse and wavelet.
  h = ww.applyB(h)
  ah = ww.getAHIter3(na,ka,nh,kh,h,niter,eps,u,f,g)
  a = ah[0]
  h = ah[1]
  ...
  slag = ww.applySLA(na,ka,a,u,g)
  plotImage(sft,sx,slag,itmin=0,itmax=501,\
  ixmin=0,ixmax=720,amax=amax,title="SLAg")
  ag = ww.applyA(na,ka,a,g)
  plotImage(sgt,sx,ag,itmin=0,itmax=501,\
  ixmin=0,ixmax=720,amax=amax,title="Ag")
  plotImage(sft,sx,hslag,itmin=itminf,itmax=itmaxf,\
  ixmin=ixmin,ixmax=ixmax,amax=amax,title="HSLAg")
  plotImage(sft,sx,f,itmin=itminf,itmax=itmaxf,\
  ...
  plotImage(sft,sx,slag,itmin=itminf,itmax=itmaxf,\
  ixmin=ixmin,ixmax=ixmax,amax=amax,title="SLAg")
  ag = ww.applyA(na,ka,a,g)
  plotImage(sgt,sx,ag,itmin=itming,itmax=itmaxg,\
  ixmin=ixmin,ixmax=ixmax,amax=amax,title="Ag")
