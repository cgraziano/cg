...
from wwarp import WaveletWarpingCBIter, Warper
    ...
    for ir1 in range(0,nr1):
      for iv in range(0,nv):
        for inoise in range(0,nnoise):
          ...
          switchc,freqd,decayd,mpd,switchd,r0[ir0],r1[ir1],v[iv],nrmsf[inoise],nrmsg[inoise],\
          ...
          #Estimate wavelet
          warp = Warper()
          ww = WaveletWarpingCBIter()
          ww.setTimeRange(tmin,tmax)
          cbw = ww.getWaveletC(niter,nb,kb,nc,kc,u,f,g)
          cw = cbw[0]
          bwn = cbw[1]
          bw = ww.getInverseB(nb,kb,nc,kc,cw,u,f,g)
          ssd = ww.getSumSqDiff()

          #Get known wavelet
          dk = getWavelet(freqd,decayd,nc,kc,mpd,switchd)
          ck = getWavelet(freqc,decayc,nc,kc,mpc,switchc)
          ...
          plotWavelets(Sampling(nb,dt,kb*dt),[nbw,nbk,nbwn],title=title3,pngDir=pngDir,onecol=True)

          maxssd = max(ssd)
          title = "Sum of square differences between f and CSBg"
          si = Sampling(niter+1,1.0,0.0)
          plotSequenceMeas(si,[ssd],amax=[maxssd+50],tmark=[maxssd/100.0],hlabel="Iterations",labels=["Sum of square diff"],pngDir=None,title=title)
          ...
          plotSequences(st,[f,csbg,g,sg,du],amax=amax,tmark=tmark,\
          labels=["f","CSGg","g","Sg","du"],pngDir=pngDir,\
          title=title)
...
def goEstimateWaveletTest2D():
  ...
  freqc,decayc,= 0.08,0.05
  mpc = True#is wavelet in f mininmum phase?
  switchc = False#does the wavelet in f have a switch length?
  freqd,decayd,= 0.08,0.05
  mpd = True#is wavelet in f mininmum phase?
  switchd = False#does the wavelet in f have a switch length?
  nrmsf = 0.00
  nrmsg = nrmsf
  ...
  for ir0 in range(0,nr0):
    for ir1 in range(0,nr1):
      for iv in range(0,nv):
        ...
        switchc,freqd,decayd,mpd,switchd,r0[ir0],r1[ir1],v[iv],\
...
def goSinopec1():
  ...
  plotUnknownWavelets(Sampling(nb,dt,kb*dt),[nbw,nbwn],title="b 1 trace",pngDir=pngDir,onecol=True)
  plotUnknownWavelets(Sampling(nd,dt,kd*dt),[ndw],title="c 1 trace",pngDir=pngDir,onecol=True)
  du = computeBackDiff(u)
  ...
  title = "Sum of square differences between f and CSBg"
  si = Sampling(niter+1,1.0,0.0)
  plotSequenceMeas(si,[ssd],amax=[maxssd],tmark=[maxssd/100.0],hlabel="Iterations",labels=["Sum of square diff"],pngDir=None,title=title)
...
def goWarpTest():
  ...
  switch = False#does the wavelet in f have a switch length?
  nrmsf = [0.5,0.1]
  nrmsg = [0.5,0.1]
  ...
  for i in range(0,nnoise):
    ...
    p,q,f,g,noisef,noiseg,u,tmin,tmax = createSyntheticLn1D(freq,decay,mp,switch,\
    r0,r1,v,nrmsf[i],nrmsg[i],nt,ni,randomi,moreps)
    #Warping f to g.
    ww = WaveletWarpingAEig()
    warp = Warper()
    hk = getWavelet(freq,decay,nc,kc,mp,switch) # known wavelet
...
def goSimpleTest():
  ...
  sfac = 1.000
  p,q,f,g,u,tmin,tmax = createSyntheticLn1D(freq,decay,mp,switch,r0,r1,c,noise,nrmsf,nrmsg,\
  nt,ni,randomi,moreps)
  #p,q,f,g,u,tmin,tmax = createSyntheticCos(freq,decay,mp,switch,r0,r1,c,noise,nrmsf,nrmsg,nt,ni)
...
#Synthetic parameters
  ...
  #Create synthetic f and g.
  p,q,f,g,noisef,noiseg,u,tmin,tmax = createSyntheticLn2D(freq,decay,mp,switch,\
...
def goNaVarySyntheticTest():
  ...
  for na in range(na0,na1,dna):
    ...
    #Estimation and sampling parameters.
    sfac = 1.000
    p,q,f,g,u,tmin,tmax = createSyntheticLn1D(freq,decay,mp,switch,r0,r1,c,\
      ...
      for ir0 in range(nr0):
        for ir1 in range(nr1):
          for ic in range(nc):
            ...
            p,q,f,g,u,tmin,tmax = createSyntheticLn1D(freq,decay,mp,switch,r0,r1,c,\
            noise,nrmsf,nrmsg,nt,ni,randomi,moreps)
            #p,q,f,g,u,tmin,tmax = createSyntheticCos(freq,decay,mp,switch,r0,r1,c,\
            #noise,nrmsf,nrmsg,nt,ni)
            #p,q,f,g,u,tmin,tmax = createSyntheticShift(freq,decay,mp,switch,c,\
            #noise,nrmsf,nrmsg,nt,ni)
            du = computeBackDiff(u)
            nt = len(f)
...
def goAcVsAa():
  ...
  #Estimation and sampling parameters.
  sfac = 1.000
  pac,qac,f,g,u,tmin,tmax = createSyntheticLn1D(freq,decay,mp,switch,r0,r1,c,nrmsf,nrmsg,\
  nt,ni,randomi,moreps)
...
def addWavelet(fpeak,decay,p,mp=False,switch=False):
  w = 2.0*PI*fpeak
  #print "w = "+str(w)
  if not mp:
    decay *= 2.0
...
def getInverseWavelet(fpeak,decay,nc,kc,na,ka,mp,switch):
  if mp:
    print "na,ka = 3,0"
...
def getWavelet(fpeak,decay,nc,kc,mp=False,swith=False):
  x = zerofloat(nc)
  x[-kc] = 1.0
  return addWavelet(fpeak,decay,x,mp,swith)
...
def testWarpingandBandPassFilter():
  ...
  randomi = False 
  moreps = False 
  p,q,f,g,noisef,noiseg,u,tmin,tmax = createSyntheticLn1D(freqc,decayc,mpc,switchc,\
  freqd,decayd,mpd,switchd,r0,r1,v,nrmsf,nrmsg,nt,ni,randomi,moreps)
  ww = WaveletWarpingAEig()
  ww.setTimeRange(tmin,tmax)
  r = r0
  nu = len(u)
