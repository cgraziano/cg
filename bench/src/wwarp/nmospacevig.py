...
pngDir = "./png/nmospacevig/"
#pngDir = None

def main(args):
  """Available CDPs 145-147, 400-800, 1100-1500, and 1700-2000"""
  cdp = [146,400,800,1100,1500,1700,2000]
  goEstimateWaveletFromGather(cdp)

def goEstimateWaveletFromGather(cdp):
  ...
  tmin,tmax,perc = 0.4,2.5,98.0

  ncdp = len(cdp)
  hpefs = zerofloat(nh,ncdp)
  hnmos = zerofloat(nh,ncdp)
  for icdp in range(0,ncdp):
    hpef,hnmo,st = inverseAndWaveletCalc(cdp[icdp],smax,fmin,fmax,texp,tbal,
        sfac,na,ka,nh,kh,tmin,tmax,perc)
    hpefs[icdp] = hpef
    hnmos[icdp] = hnmo



  title = "CDP Wavelets cdp "
  for c in range(0,ncdp):
    title = title+" "+str(cdp[c])+", "
  ...
  plotCDPWavelets(Sampling(nh,st.delta,kh*st.delta),hpefs,hmax=hmax,
               title=titlePEF,pngDir=pngDir)
  plotCDPWavelets(Sampling(nh,st.delta,kh*st.delta),hnmos,hmax=hmax,
...
def plotCDPWavelets(st,hs,hmax,title=None,pngDir=None):
  sp = SimplePlot()
  ls = [PointsView.Line.SOLID,PointsView.Line.DASH,PointsView.Line.DOT]
...
def getVikingGrabenCDP(icdp):
  if 145 <= icdp | icdp <= 147:
    ...
    print "Requested CDP not available: "+str(icdp)
